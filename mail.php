...
##-----------------------------------------------------------------------------------------------------------------##
#
#  【MultiLangForm】PHPメールプログラム　多言語対応版 最終更新日2014/12/25　（文字コードはUTF-8のみ可）
...
$site_top = "http://yamazakinami.webcrow.jp/";

// 管理者メールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
$to = "na73gigne@gmail.com";
...
$Referer_check_domain = "yamazakinami.webcrow.jp";
...
$thanksPage = "http://yamazakinami.webcrow.jp/thanks.html";
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	if($remail == 1) {
		if (ini_get('safe_mode')) {
		  ...
		  $result = mb_send_mail($post_mail,$re_subject,$userBody,$reheader,'-f'. $to);
		}
	}
}
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".h($key)."】".$requireError['text']."</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">【".$requireVal."】".$requireError['select']."</p>\n";
				$res['empty_flag'] = 1;
		}
	}
